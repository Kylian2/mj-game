...
function Posters({
    ...
    posters: Array<{ title: string; src: string; scene?: string; training?: boolean }>;
    setScene: Dispatch<SetStateAction<string>>;
}) {
    ...
    return (
        <group position={[0, 2, 0]}>
            {posters.map((poster, index) => {
                const angle = (index / Math.max(posterCount, 8)) * Math.PI * 2 + Math.PI;
                ...
                return (
                    <group key={index}>
                        <PosterSpotLight targetPosition={[x, 0, z]} />
                        <Poster
                            title={poster.title}
                            image={poster.src}
                            scene={poster.scene}
...
export function DiscoSpotlight({
    position,
    color,
    speed = 1,
    pattern = "circle"
...
export function HomeScene({ scene }: { scene: [string, Dispatch<SetStateAction<string>>] }) {
    const posters = [
        {
            title: "Danube Bleu",
            src: "posters/danubebleu.jpg"
        },
        {
            title: "Deux balles",
            src: "posters/auclairdelalune.jpg",
            scene: "two-balls",
            training: true
        },
        { title: "Tutoriel", src: "posters/tutorial.png", scene: "tutorial" },
        {
            title: "Trois Balles",
            src: "posters/pachelbel.jpg",
            scene: "three-balls",
            training: true
        }
    ];
    ...
    return (
        <group>
            ...
            />
            <SpotlightObject
                position={new Vector3(Math.cos(0) * 4, 6, Math.sin(0) * 4)}
                rotation={new Euler(0, Math.PI / 2, Math.PI / 7)}
            />
            <SpotlightObject
                position={
                    new Vector3(Math.cos((2 * Math.PI) / 3) * 4, 6, Math.sin((2 * Math.PI) / 3) * 4)
                }
                rotation={new Euler(0, Math.PI / 4, -Math.PI / 5)}
            />
            <SpotlightObject
                position={
                    new Vector3(Math.cos((4 * Math.PI) / 3) * 4, 6, Math.sin((4 * Math.PI) / 3) * 4)
                }
